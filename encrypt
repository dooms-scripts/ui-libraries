		...
		function tab.new_group(group_name)
			...
			function group.new_category(title_text)
				...
				function category.new_text(text_input)
					local text_divider = {}
					local text_input = text_input or 'text label'
					...
					function text_divider:get_text()
						return text.Text
					end
				...
				function category.new_toggle(text_input, yield_color, callback)
					local toggle = {}
					toggle.value = false

					local text_input = text_input or 'toggle'
					local yield_color = yield_color or false

					if type(yield_color) == 'function' then
						callback = yield_color
						yield_color = false
					end
				...
				function category.new_button(text_input, callback)
					...
					local text_input = text_input or 'button'
				...
				function category.new_textbox(text_input, placeholder_text, callback)
					...
					local text_input = text_input or 'textbox'
					local placeholder_text = placeholder_text or '...'
				...
				function category.new_keybind(text_input, default_key, callback)
					...
					local text_input = text_input or 'keybind'
					local default_key = default_key or '...'

					if type(default_key) == 'function' then
						callback = default_key
						default_key = '...'
					end
					...
					input_service.InputBegan:Connect(function(input)
						if typing then return warn('typing; returned')
						if editing == true then and not typing then
				...
				function category.new_slider(text, default_value, min_value, max_value, allow_decimals, callback)
					...
					local default_value = default_value or 0
					local min_value = min_value or 0
					local max_value = max_value or 100
					local allow_decimals = allow_decimals or false

					if type(allow_decimals) == 'function' then
						callback = allow_decimals
