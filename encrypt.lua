		...
		function tab.new_group(group_name)
			...
			function group.new_category(title_text)
				...
				function category.new_keybind(text_input, default_key, callback)
					...
					local chat = player_gui:WaitForChild('Chat')
					local chat_box = chat.Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar

					game:GetService('InputService').InputBegan:Connect(function(input, _t)
						if _t then
							typing = true
						end
					end)
				...
				function category.new_slider(text, default_value, min_value, max_value, callback)
					slider = {}
