...
-- [ ATTRIBUTES ] ----------------------------------------------------------------------------------
local esp_library = { version = 'e1.0.7' }
...
function esp_library.new(esp_type, player, ...)	
	if esp_type == 'highlight' then
		local highlight, defaults, connections = {}, {
			enabled = true,
		...
		local instance = Instance.new('Highlight', cloneref(game.CoreGui))
		
		local function create_highlight()
			local character = player.Character or player.CharacterAdded:Wait()
			instance.DepthMode = 'AlwaysOnTop'
			instance.FillColor = data.fill_color
			instance.OutlineColor = data.outline_color
			instance.FillTransparency = data.fill_transparency
			instance.OutlineTransparency = data.outline_transparency
		...
		function highlight:Update(player, ...)
			local data = overwrite(defaults, ... or {})
			instance.FillColor = data.fill_color
			instance.OutlineColor = data.outline_color
			instance.FillTransparency = data.fill_transparency
	...
	if esp_type == 'text' then
		local defaults = { -- <table>
			color = Color3.fromRGB(255, 255, 255), -- <color3>
			outline_color = Color3.fromRGB(0, 0, 0), -- <color3>
			update_speed = 0, -- <number>
			outline = false, -- <bool>
		}
...
function encrypt_esp.new_highlight(player, properties)
	...
	local function create_highlight()
		pcall(function()
			...
			highlight_instance.Adornee = char
			highlight_instance.Enabled = true
			highlight_instance.FillColor = color
